...
Technically, this is a two-turn generation with persistent steering. Turn 1: generate the answer with the steering hook active. Turn 2: append the answer and the confidence prompt to the context, generate again with the same hook still active (the hook only fires during prefill because of the L<=1 guard, so both turns see the steered activations). Parse the numeric response with regex and clamp to [0, 1].
...
2. The MCMC acceptance rate (from Method 4) is a useful uncertainty signal on its own: high acceptance = the oracle is uncertain.
